...
import {loader, afterLoader} from "./loadingFunc/loadingFunc";
...
function App() {
    ...
    function onMessageWS(parsedData) {
        console.log("message")
        for (const parsedDataKey in parsedData) {
            if (parsedData.hasOwnProperty(parsedDataKey)) {
                defVal[parsedDataKey] = parsedData[parsedDataKey];
            }
        }
        setServerData({...defVal});
    ...
    // let colors = [{eng: "red", rus: "Красный"}, {eng: "white", rus: "Белый"}]
    ...
    function connect() {
        ...
        ws.current.onclose = event => {
            ...
            setOnline(false)
            ws.current.close() // почему то закоменчено было, не помню
            !isDev && connect()
        }
        ws.current.onmessage = event => {
            let parsedData = JSON.parse(event.data)
            onMessageWS(parsedData)
        }
    ...
    const getFilteredData = () => {
        const timeArray = serverData.Br_time;
        const validIndices = [];

        // Фильтруем остальные элементы
        for (let i = 0; i < timeArray.length; i++) {
            if (i == 0 && timeArray[i] == "00:00") {
                validIndices.push(i);
                continue;
            }
            if (timeArray[i] !== "00:00") {
                validIndices.push(i);
            }
        }

        return {
            time: validIndices.map(i => timeArray[i]),
            // channels: ["Br1","Br4"].map(channel =>
            channels: ["Br1", "Br2", "Br3", "Br4", "Br5", "Br6", "Br7", "Br8"].map(channel =>
                validIndices.map(i => serverData[channel][i]))
        };
    };

    const prepareDatasets = () => {
        const filtered = getFilteredData();
        return filtered.channels.map((channelData, idx) => ({
            label: colors[idx].rus,
            data: channelData.map((value, i) => ({
                x: parseTime(filtered.time[i]),
                y: value
            })),
            borderColor: colors[idx].eng,
            borderWidth: 1,
            backgroundColor: colors[idx].eng,
        }));
    };
    ...
    // Обновляем конфигурацию данных и опций
    const data = {
        ...
        <Storage.Provider value={{serverData}}>
            ...
            <div className="main"
                ...
                <div className={"footerNav"}>
                    ...
                    <button onClick={(ev) => {
                        sync(ev)
                    }} title={"Синхронизация других светильников"}>
                        <div className={"loaderr"}></div>
                        <svg width="30px" height="30px" viewBox="0 0 24.00 24.00" fill="none"
                             xmlns="http://www.w3.org/2000/svg" stroke="#ffffff" transform="rotate(45)">
                            <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
